...
def main():
    ...
    t = Timer()
    t.start()
    solver = CNFSolverGenetic(sentence,n=50,size=50,mutationRate=0.5,generations=500)
    x = solver.getSolution()
    print("Roll No : 2019A7PS0160G")
    print('Number of clauses in CSV file : ',len(sentence))
    print('Best model : ',x[1])
    print('Fitness Value of best model : ',CNFEvaluator.getHueristicScore(sentence,x[1])*100/len(sentence),'%')
